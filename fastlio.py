...
#   内联展开于 predict_inplace() 之中，不再以独立函数形式存在。
#
# 【对应 C++ 文件】
...
# Cached zero 2-vec used inside predict_inplace() to avoid reallocating each call.
...
def update_iterated(
    ...
    for i in range(-1, max_iter):
        ...
        if t_converge > 1 or i == max_iter - 1:
            # Final covariance update — K_x_12 came from the np.linalg.solve
            # above, so it lives in sol13's first 12 columns. We must finish
            # using it before the next fill, but since we return immediately
            # below that's fine.
            np.copyto(L_buf, P)
            L = L_buf
...
#     → 轨迹记录 / 逐帧去畸变点云与位姿写盘（frames/，按需重建地图）
#
# 关键数据结构：
